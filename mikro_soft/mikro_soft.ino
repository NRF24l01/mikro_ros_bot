/****************************************************************
 * PWM + Wi-Fi + AsyncWebServer + PCNT-энкодеры + ОДОМЕТРИЯ + PID-регулятор
 * Управление и мониторинг по Wi-Fi (JSON API)
 * PID коэффициенты сохраняются в SPIFFS и загружаются при старте.
 * + Battery Voltage Monitoring
 ****************************************************************/
#include <WiFi.h>
#include <ESPAsyncWebServer.h>
#include <AsyncTCP.h>
#include "driver/pcnt.h"
#include <math.h>
#include <FS.h>
#include <SPIFFS.h>
#include <ArduinoJson.h>
#include "driver/adc.h" // <<< ADDED: For explicit ADC configuration

/* ── Аппаратные параметры ─────────────────────────────*/
#define L_A 32
#define L_B 33
#define R_A 26
#define R_B 25
#define ENC_R_A 18
#define ENC_R_B 19
#define ENC_L_A 35
#define ENC_L_B 34

#define BATT_PIN 39      // <<< ADDED: GPIO39 for battery voltage (ADC1_CH3)
const float R1_BATT = 470000.0f; // <<< ADDED: 470k Ohm resistor for voltage divider
const float R2_BATT = 235000.0f; // <<< ADDED: 235k Ohm resistor for voltage divider


constexpr float WHEEL_RADIUS = 0.0223f; // метры
constexpr float BASE         = 0.0956f; // метры (ширина между колесами)
constexpr int   TICKS_PER_TURN = 2936;  // тиков энкодера на оборот

constexpr char SSID[] = "robotx";
constexpr char PASS[] = "78914040";

/* ── Глобальные значения ──────────────────────────────*/
volatile uint8_t dutyLA = 0, dutyLB = 0, dutyRA = 0, dutyRB = 0;
volatile int32_t encTotalL = 0, encTotalR = 0;
volatile float currentBatteryVoltage = 0.0f; // <<< ADDED: To store current battery voltage

AsyncWebServer server(80);

/* ── ОДОМЕТРИЯ ───────────────────────────────────────*/
struct Odom {
  float x = 0, y = 0, theta = 0;
  float v = 0, w = 0;
} odom;

/* ── Скорости колес ──────────────────────────────────*/
struct WheelSpeed {
  float left = 0, right = 0; // мм/с
} wheelSpeed;

/* ── Инвертированный вид: V, W как целевые для регулятора ── */
struct VWTarget {
  float v = 0; // мм/с
  float w = 0; // рад/с
} vwTarget;

/* ── ПИД-регулятор ───────────────────────────────────*/
struct PID {
  float kp = 2.0, ki = 2.5, kd = 0.0, kff = 0.3;
  float sumL = 0, sumR = 0, prevErrL = 0, prevErrR = 0;
  float outL = 0, outR = 0;
  float targetL = 0, targetR = 0; // мм/с
} pid;

// PID config file path in SPIFFS
const char* PID_CONFIG_FILE = "/pid.json";

// <<< ADDED: Function to read and calculate battery voltage
float readBatteryVoltage() {
  int adc_raw = adc1_get_raw(ADC1_CHANNEL_3); // GPIO39 is ADC1_CH3
  // Assuming ESP32 ADC 12-bit (0-4095).
  // With ADC_ATTEN_DB_11, the effective voltage range is approx 0-3.1V (can vary).
  // We'll use 3.1V as the Vref for full scale reading for this attenuation. Calibrate if needed.
  float v_adc_pin = adc_raw * (3.3f / 4095.0f);
  float v_battery = v_adc_pin * (R1_BATT + R2_BATT) / R2_BATT;
  // Basic filter (simple moving average over 2 readings) - optional
  static float last_voltage = v_battery;
  v_battery = (v_battery + last_voltage) / 2.0f;
  last_voltage = v_battery;
  return v_battery;
}


void savePIDToSPIFFS() {
  DynamicJsonDocument doc(256);
  doc["kp"] = pid.kp;
  doc["ki"] = pid.ki;
  doc["kd"] = pid.kd;
  doc["kff"] = pid.kff;
  File file = SPIFFS.open(PID_CONFIG_FILE, FILE_WRITE);
  if (file) {
    serializeJson(doc, file);
    file.close();
  }
}

void loadPIDFromSPIFFS() {
  if (!SPIFFS.exists(PID_CONFIG_FILE)) return;
  File file = SPIFFS.open(PID_CONFIG_FILE, FILE_READ);
  if (file) {
    DynamicJsonDocument doc(256);
    if (deserializeJson(doc, file) == DeserializationError::Ok) {
      if (doc.containsKey("kp")) pid.kp = doc["kp"];
      if (doc.containsKey("ki")) pid.ki = doc["ki"];
      if (doc.containsKey("kd")) pid.kd = doc["kd"];
      if (doc.containsKey("kff")) pid.kff = doc["kff"];
    }
    file.close();
  }
}

/* ── PWM ─────────────────────────────────────────────*/
void analogWriteTrack(uint8_t pin, uint8_t duty) {
  analogWrite(pin, duty);
  switch (pin) {
    case L_A: dutyLA = duty; break;  case L_B: dutyLB = duty; break;
    case R_A: dutyRA = duty; break;  case R_B: dutyRB = duty; break;
  }
}
void stopMotors() {
  analogWriteTrack(L_A,0); analogWriteTrack(L_B,0);
  analogWriteTrack(R_A,0); analogWriteTrack(R_B,0);
}

/* ── PCNT ────────────────────────────────────────────*/
void setupEncoder(pcnt_unit_t unit, gpio_num_t a, gpio_num_t b) {
  pcnt_config_t c{};
  c.pulse_gpio_num=a; c.ctrl_gpio_num=b; c.unit=unit; c.channel=PCNT_CHANNEL_0;
  c.pos_mode=PCNT_COUNT_INC; c.neg_mode=PCNT_COUNT_DEC;
  c.lctrl_mode=PCNT_MODE_REVERSE; c.hctrl_mode=PCNT_MODE_KEEP;
  c.counter_h_lim=32767; c.counter_l_lim=-32768;
  pcnt_unit_config(&c);
  pcnt_set_filter_value(unit,100); pcnt_filter_enable(unit);
  pcnt_counter_clear(unit); pcnt_counter_resume(unit);
}
inline int16_t snapPCNT(pcnt_unit_t u){
  int16_t v; pcnt_get_counter_value(u,&v); pcnt_counter_clear(u); return v;
}

/* ── ПИД-регулятор ───────────────────────────────────*/
float constrainf(float x, float a, float b) { return x < a ? a : (x > b ? b : x); }

void pidUpdate(float dt, float vL, float vR) {
  // --- Левое колесо
  float errL = pid.targetL - vL;
  pid.sumL += errL * dt;
  pid.sumL = constrainf(pid.sumL, -500, 500); // Anti-windup for sum
  float dErrL = (errL - pid.prevErrL) / dt;
  float ffL = pid.kff * pid.targetL;
  pid.outL = pid.kp*errL + pid.ki*pid.sumL + pid.kd*dErrL + ffL;
  pid.prevErrL = errL;
  // --- Правое колесо
  float errR = pid.targetR - vR;
  pid.sumR += errR * dt;
  pid.sumR = constrainf(pid.sumR, -500, 500); // Anti-windup for sum
  float dErrR = (errR - pid.prevErrR) / dt;
  float ffR = pid.kff * pid.targetR;
  pid.outR = pid.kp*errR + pid.ki*pid.sumR + pid.kd*dErrR + ffR;
  pid.prevErrR = errR;
  // --- Ограничение PWM [0..255]
  pid.outL = constrainf(pid.outL, -255, 255);
  pid.outR = constrainf(pid.outR, -255, 255);

  // --- Драйверы H-моста: прямое/обратное направление
  if (pid.outL > 0) {
    analogWriteTrack(L_A, (uint8_t)pid.outL); analogWriteTrack(L_B, 0);
  } else {
    analogWriteTrack(L_A, 0); analogWriteTrack(L_B, (uint8_t)(-pid.outL));
  }
  if (pid.outR > 0) {
    analogWriteTrack(R_A, (uint8_t)pid.outR); analogWriteTrack(R_B, 0);
  } else {
    analogWriteTrack(R_A, 0); analogWriteTrack(R_B, (uint8_t)(-pid.outR));
  }
}

/* ── ОДОМЕТРИЯ+СКОРОСТЬ ─────────────────────────────*/
void updateOdometryAndSpeed(int32_t dTicksL, int32_t dTicksR, float dt) {
  float dL = (float)dTicksL * 2.0f * M_PI * WHEEL_RADIUS / TICKS_PER_TURN;
  float dR = (float)dTicksR * 2.0f * M_PI * WHEEL_RADIUS / TICKS_PER_TURN;

  // --- скорость колёс [мм/с]
  wheelSpeed.left  = (dt > 0.0001f) ? (dL * 1000.0f / dt) : 0.0f;
  wheelSpeed.right = (dt > 0.0001f) ? (dR * 1000.0f / dt) : 0.0f;

  // --- одометрия
  float dCenter = (dL + dR) * 0.5f;
  float dTheta = (dR - dL) / BASE;
  odom.x += dCenter * cosf(odom.theta + dTheta/2.0f);
  odom.y += dCenter * sinf(odom.theta + dTheta/2.0f);
  odom.theta += dTheta;
  if (dt > 0.0001f) { // Avoid division by zero if dt is too small
    odom.v = dCenter / dt;
    odom.w = dTheta / dt;
  } else {
    odom.v = 0;
    odom.w = 0;
  }
}

/* ── Прямое и обратное преобразование V, W <-> скорости колес ── */
void set_targets_from_vw(float v_mm_s, float w_rad_s) {
  // V в мм/с, W в рад/с
  // V_L = V - (BASE/2)*W; V_R = V + (BASE/2)*W
  pid.targetL = v_mm_s - (BASE * 500.0f) * w_rad_s; // BASE/2*1000=WHEEL_BASE/2 в мм
  pid.targetR = v_mm_s + (BASE * 500.0f) * w_rad_s;
  vwTarget.v = v_mm_s;
  vwTarget.w = w_rad_s;
}
void update_vw_from_targets() {
  // обратное преобразование для отображения
  vwTarget.v = (pid.targetL + pid.targetR) * 0.5f;
  vwTarget.w = (pid.targetR - pid.targetL) / (BASE * 1000.0f); // BASE needs to be in mm here
}

/* ── Wi-Fi + Web ─────────────────────────────────────*/
void setupWiFiAndRoutes() {
  WiFi.mode(WIFI_STA); WiFi.begin(SSID,PASS);
  Serial.print("Connecting");
  while(WiFi.status()!=WL_CONNECTED){ Serial.print('.'); delay(400);}
  Serial.printf("\nIP: %s\n",WiFi.localIP().toString().c_str());

  server.on("/state",HTTP_GET,[](AsyncWebServerRequest *req){
    update_vw_from_targets();
    currentBatteryVoltage = readBatteryVoltage(); // <<< MODIFIED: Read current battery voltage
    char js[750]; // Increased buffer size slightly
    snprintf(js,sizeof(js),
      "{\"duty\":{\"L_A\":%u,\"L_B\":%u,\"R_A\":%u,\"R_B\":%u},"
      "\"enc\":{\"left\":%ld,\"right\":%ld},"
      "\"odom\":{\"x\":%.4f,\"y\":%.4f,\"theta\":%.4f,\"v\":%.4f,\"w\":%.4f},"
      "\"speed\":{\"left\":%.1f,\"right\":%.1f},"
      "\"pid\":{\"kp\":%.3f,\"ki\":%.3f,\"kd\":%.3f,\"kff\":%.3f,"
      "\"targetL\":%.1f,\"targetR\":%.1f},"
      "\"vw\":{\"v\":%.1f,\"w\":%.3f},"
      "\"pwm\":{\"L\":%.1f,\"R\":%.1f},"
      "\"battery_v\":%.2f}", // <<< ADDED: battery voltage field
      dutyLA,dutyLB,dutyRA,dutyRB, encTotalL,encTotalR,
      odom.x, odom.y, odom.theta, odom.v, odom.w,
      wheelSpeed.left, wheelSpeed.right,
      pid.kp, pid.ki, pid.kd, pid.kff, pid.targetL, pid.targetR,
      vwTarget.v, vwTarget.w,
      pid.outL, pid.outR,
      currentBatteryVoltage // <<< ADDED: battery voltage value
    );
    req->send(200,"application/json",js);
  });

  // Получить только PID коэффициенты (для быстрой инициализации GUI)
  server.on("/pid", HTTP_GET, [](AsyncWebServerRequest *req){
    char js[128];
    snprintf(js, sizeof(js),
      "{\"kp\":%.3f,\"ki\":%.3f,\"kd\":%.3f,\"kff\":%.3f}",
      pid.kp, pid.ki, pid.kd, pid.kff
    );
    req->send(200, "application/json", js);
  });

  // <<< ADDED: Endpoint to get battery voltage
  server.on("/battery", HTTP_GET, [](AsyncWebServerRequest *req){
    currentBatteryVoltage = readBatteryVoltage();
    char js[64];
    snprintf(js, sizeof(js), "{\"voltage\":%.2f}", currentBatteryVoltage);
    req->send(200, "application/json", js);
  });

  // Установка коэффициентов PID
  server.on("/setPID", HTTP_GET, [](AsyncWebServerRequest *req){
    bool changed = false;
    if(req->hasParam("kp")) { pid.kp = req->getParam("kp")->value().toFloat(); changed = true; }
    if(req->hasParam("ki")) { pid.ki = req->getParam("ki")->value().toFloat(); changed = true; }
    if(req->hasParam("kd")) { pid.kd = req->getParam("kd")->value().toFloat(); changed = true; }
    if(req->hasParam("kff")) { pid.kff = req->getParam("kff")->value().toFloat(); changed = true; }
    if (changed) savePIDToSPIFFS();
    req->send(200,"text/plain","PID updated");
  });

  // Установка целевых скоростей колес
  server.on("/setWheelsSpeed", HTTP_GET, [](AsyncWebServerRequest *req){
    if(req->hasParam("left"))  pid.targetL = req->getParam("left")->value().toFloat();
    if(req->hasParam("right")) pid.targetR = req->getParam("right")->value().toFloat();
    update_vw_from_targets();
    req->send(200,"text/plain","Targets updated");
  });

  // Установка целевых V и W
  server.on("/setVW", HTTP_GET, [](AsyncWebServerRequest *req){
    float v = req->hasParam("v") ? req->getParam("v")->value().toFloat() : 0.0f;
    float w = req->hasParam("w") ? req->getParam("w")->value().toFloat() : 0.0f;
    set_targets_from_vw(v, w);
    stopMotors(); // <<< ADDED: Stop motors briefly before new command if v and w are zero
    if (v == 0.0f && w == 0.0f) {
        pid.targetL = 0; pid.targetR = 0; // ensure targets are zero
        pid.sumL = 0; pid.sumR = 0; // Reset PID sums on stop
        pid.outL = 0; pid.outR = 0;
    }
    req->send(200,"text/plain","VW targets updated");
  });

  // Сброс всех значений (энкодеры, одометрия, PID-интеграторы)
  server.on("/resetAll", HTTP_GET, [](AsyncWebServerRequest *req){
    encTotalL=encTotalR=0;
    odom = Odom(); wheelSpeed = WheelSpeed();
    pid.sumL = 0; pid.sumR = 0; pid.prevErrL = 0; pid.prevErrR = 0;
    pid.outL = 0; pid.outR = 0;
    pid.targetL = 0; pid.targetR = 0;
    vwTarget.v = 0; vwTarget.w = 0;
    stopMotors(); // Ensure motors are stopped
    pcnt_counter_clear(PCNT_UNIT_0); pcnt_counter_clear(PCNT_UNIT_1);
    req->send(200,"text/plain","All reset");
  });

  server.on("/", HTTP_GET, [](AsyncWebServerRequest *request) {
    request->send(200, "text/plain",
        "OK. Use /state, /pid, /battery, /setPID, /setWheelsSpeed, /setVW, /resetAll");
  });
  server.begin();
}

/* ── SETUP ───────────────────────────────────────────*/
void setup() {
  Serial.begin(115200);

  // <<< ADDED: ADC Configuration for Battery Pin (GPIO39 is ADC1_CH3)
  // pinMode(BATT_PIN, INPUT); // analogRead/adc1_get_raw usually handles mode.
  adc1_config_width(ADC_WIDTH_BIT_12);             // 12-bit resolution
  adc1_config_channel_atten(ADC1_CHANNEL_3, ADC_ATTEN_DB_11); // Attenuation for ~0-3.1V input range

  // SPIFFS init
  if (!SPIFFS.begin(true)) {
    Serial.println("SPIFFS mount failed!");
  } else {
    loadPIDFromSPIFFS();
  }

  pinMode(L_A,OUTPUT); pinMode(L_B,OUTPUT);
  pinMode(R_A,OUTPUT); pinMode(R_B,OUTPUT); stopMotors();

  pinMode(ENC_R_A,INPUT); pinMode(ENC_R_B,INPUT);
  pinMode(ENC_L_A,INPUT); pinMode(ENC_L_B,INPUT);
  setupEncoder(PCNT_UNIT_0,(gpio_num_t)ENC_R_A,(gpio_num_t)ENC_R_B);
  setupEncoder(PCNT_UNIT_1,(gpio_num_t)ENC_L_A,(gpio_num_t)ENC_L_B);

  setupWiFiAndRoutes();
  currentBatteryVoltage = readBatteryVoltage(); // Initial read
  Serial.printf("Initial Battery Voltage: %.2f V\n", currentBatteryVoltage);
}

/* ── LOOP ────────────────────────────────────────────*/
void loop() {
  static uint32_t tEnc=0;
  static uint32_t lastUpdate = 0;
  uint32_t now = millis(); // Renamed to 'now' for clarity within loop scope
  
  if(now-tEnc>=10){ // Encoder and PID update loop (100Hz)
    tEnc=now;
    int16_t dR = snapPCNT(PCNT_UNIT_0);
    int16_t dL = snapPCNT(PCNT_UNIT_1);
    encTotalR += dR;
    encTotalL += dL;
    float dt = (now-lastUpdate)/1000.0f;
    if(dt > 0.0001f) { // Ensure dt is valid
      updateOdometryAndSpeed(dL, dR, dt);
      // Only call pidUpdate if targets are non-zero or motors haven't settled
      if (vwTarget.v != 0.0f || vwTarget.w != 0.0f || pid.outL !=0 || pid.outR !=0 ) {
         pidUpdate(dt, wheelSpeed.left, wheelSpeed.right);
      } else { // If targets are zero and outputs are zero, ensure motors are off.
         stopMotors();
      }
      lastUpdate = now;
    }
  }

  // Печать состояния и батареи раз в 500 мс
  static uint32_t tLog=0;
  if(now-tLog>=500){ // Logging loop (2Hz)
    tLog=now;
    Serial.printf("SPD L=%.1f mm/s R=%.1f mm/s Target L=%.1f mm/s R=%.1f mm/s\n", 
      wheelSpeed.left, wheelSpeed.right, pid.targetL, pid.targetR);
    Serial.printf("VW: V=%.1f mm/s W=%.3f rad/s. PWM L:%.1f R:%.1f\n", vwTarget.v, vwTarget.w, pid.outL, pid.outR);
    currentBatteryVoltage = readBatteryVoltage(); // <<< MODIFIED: Read battery voltage
    Serial.printf("Battery Voltage: %.2f V\n", currentBatteryVoltage); // <<< MODIFIED: Print battery voltage
  }
}